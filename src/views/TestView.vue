<template>
  ...
  <Button label="get users" @click="handleGetUsers"></Button>
  <div v-for="user in usersStore.users" :key="user._id" class="flex gap-1">
    <div class="text-blue-400">{{ user.username }}</div>
    \
    <div class="text-red-400">{{ user.email }}</div>
  </div>
</template>
